...
#' @export
#'
bootstrap_lpi_citation <- function(SpeciesLambdaArray, fileindex, DSize, Group, Weightings, use_weightings, use_weightings_B, WeightingsB, CAP_LAMBDAS,
                          MIN_SPECIES_PER_YEAR) {

  unique_files <- unique(fileindex)
  ...
  for (J in 2:DSize) {
    ...
    for (FileNo in resampled_files) {
      ...
      if(!is.null(SpeciesLambda)) {
        ...
        if(!is.null(SpeciesLambdaVal)) {
          SpeciesLambdaVal = na.omit(SpeciesLambdaVal)

          # Calculate the mean/median for this source
          if (length(SpeciesLambdaVal) >= MIN_SPECIES_PER_YEAR) {
            # This maintains the Weighting logic from your original script
            if (use_weightings) {
              D[GroupNo] = D[GroupNo] + median(SpeciesLambdaVal)*Weightings[[1]][FileNo]
            } else {
              D[GroupNo] = D[GroupNo] + median(SpeciesLambdaVal)
            }
